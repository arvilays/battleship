...
export default class Battleship {
  constructor() {
    ...
    this.startSolo.addEventListener("click", () => {
      if (!this.gameStarted) this.startGame("solo");
    });
    this.startVersus.addEventListener("click", () => {
  ...
  startGame(gameMode) {
    ...
    this.playerOne.isCurrentTurn =
      gameMode === "solo" ? false : Math.random() < 0.5;
    ...
    if (this.playerOne.isCurrentTurn) {
      this.playerOne.gameboardDOM.querySelector(".board-grid").style.boxShadow =
        "0px 0px 50px red";
    } else {
      this.playerTwo.gameboardDOM.querySelector(".board-grid").style.boxShadow =
        "0px 0px 50px red";
    }
    ...
    for (let i = 0; i < ships.length; i++) {
      ...
      for (let j = 0; j < shipCoords.length; j++) {
        ...
        if (shipBox.hit && !force) {
          if (shipBox.ship.isSunk())
            this.#colorShip(player, shipCoords[j], "darkred");
          else this.#colorShip(player, shipCoords[j], "red");
        } else
          this.#colorShip(player, shipCoords[j], colors[i % colors.length]);
      }
    }
  }
